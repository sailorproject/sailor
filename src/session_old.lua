...
local serialize = require"src.serialize".serialize
...
function M.open ()
	...
	local mkurlpath = utils.mkurlpath
	function utils.mkurlpath (script, data)
		if not data then
			data = {}
		end
		data[ID_NAME] = id
	...
	--[[id = sailor.session[ID_NAME] or M.new()
	if id then
		sailor.session[ID_NAME] = nil
		M.data = M.load (id) or {}
	end]]
