...
es_model.new = function(model_name)
	...
	self.save = function(arg)
		if arg.doc == nil then 
			return es.index(es_type, arg.id, attributes, arg.params, arg.index)
		else
			return es.index(es_type, arg.id, arg.doc, arg.params, arg.index)
		end
	end
	--  Function for getting data from ES
	self.get = function(arg)
		return es.get(es_type, arg.id, arg.params, arg.index)
	end
	-- Function for searching ES
	self.search = function(arg)
		return es.search(es_type, arg.query, arg.doc, arg.params, arg.index)
	end
	-- Function for delete documents in ES
	self.delete = function(arg)
		return es.delete(es_type, arg.id, arg.params, arg.index)
	end
	-- Function for updating documents in ES
	self.update = function(arg)
		return es.update(es_type, arg.id, arg.doc, arg.params, arg.index)
	end
